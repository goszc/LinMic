...
JNIEXPORT jint JNICALL Java_org_linmic_app_NativeAudio_configure(JNIEnv* e,jobject,jlong h,jstring host,jint port,jint bitrate,jint frame,jlong session,jbyteArray key,jint device){
 ...
 return engine->configure(hostname,port,bitrate,frame,static_cast<uint32_t>(session),bytes,device);
}
...
JNIEXPORT void JNICALL Java_org_linmic_app_NativeAudio_stats(JNIEnv* e,jobject,jlong h,jfloatArray array){if(!h||!array)return;jfloat data[144]{};reinterpret_cast<AudioEngine*>(h)->snapshot(data,144);e->SetFloatArrayRegion(array,0,std::min(144,e->GetArrayLength(array)),data);}
}
extern "C" {
