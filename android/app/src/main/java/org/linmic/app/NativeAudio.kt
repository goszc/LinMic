...
object NativeAudio {
    ...
    external fun configure(handle: Long, host: String, port: Int, bitrate: Int, frame: Int, session: Long, key: ByteArray, device: Int): Int
...
class NativeAudioController {
    ...
    @Synchronized fun start(host: String, port: Int, bitrate: Int, frame: Int, session: Long, key: ByteArray, device: Int, gain: Float, muted: Boolean) {
        stop()
        handle = NativeAudio.create()
        try {
            check(NativeAudio.configure(handle, host, port, bitrate, frame, session, key, device) == 0) { "Audio configuration failed" }
